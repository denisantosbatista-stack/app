...
export default function LibraryCard({ palette, index, onToggleFavorite, onExport, onDelete }) {
  ...
  return (
    <motion.div
      ...
      className="group bg-ink-surface rounded-sm overflow-hidden border border-black/[0.06] hover:border-gold/40 transition-all duration-500 hover:shadow-gold"
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="min-w-0">
            ...
            <div className="text-xs text-zinc-600 truncate">{palette.description}</div>
          </div>
          {palette.source === "ai" && (
          ...
          <div className="flex gap-1 flex-wrap">
            {(palette.tags || []).slice(0, 2).map((t) => (
              <span
                ...
                className="text-[9px] uppercase tracking-wider text-zinc-600 px-1.5 py-0.5 border border-black/10 rounded-sm"
              >
                {t}
              </span>
            ))}
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className={`p-1.5 rounded-sm transition-colors ${
                palette.favorite ? "text-gold" : "text-zinc-500 hover:text-ink-text"
              }`}
            ...
            <button
              ...
              className="p-1.5 rounded-sm text-zinc-500 hover:text-ink-text"

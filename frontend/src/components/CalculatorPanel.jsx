...
/**
 * CalculatorPanel — versão reutilizável da Calculadora (proporções, precificação, medidas 3D).
 * Pode ser embutido em qualquer página (ex.: drawer da Studio) ou usado como conteúdo
 * principal pela página /calculadora/* via wrapper.
 *
 * Props:
 *  - mode (controlado): "volume" | "pricing" | "measure"
 *  - onModeChange(nextMode): callback ao trocar de aba (para sincronizar URL na página /calculadora)
 *  - initialMode (não controlado): aba inicial quando `mode` não for fornecido
 *  - compact: layout mais denso (lg:grid-cols-1) — útil para drawer estreito
 */
export default function CalculatorPanel({ mode: modeProp, onModeChange, initialMode = "volume", compact = false }) {
  const [internalMode, setInternalMode] = useState(initialMode);
  const isControlled = typeof modeProp === "string";
  ...
  return (
    <div data-testid="calculator-panel">
      ...
      {mode === "pricing" && <PricingMode compact={compact} />}
      {mode === "measure" && <MeasureMode compact={compact} />}
    </div>
  );
}
...
function gridCols(compact) {
  return compact ? "grid gap-6" : "grid lg:grid-cols-2 gap-6";
}

function VolumeMode({ compact }) {
  ...
  return (
    <div className={gridCols(compact)}>
...
function PricingMode({ compact }) {
  const [volume, setVolume] = useState(50);
  const [resinCostL, setResinCostL] = useState(450);
  const [pigmentCostL, setPigmentCostL] = useState(2000);
  ...
  const calc = useMemo(() => {
    ...
    return { matResina, matPigmento, materialTotal, trabalho, custoTotal, precoVenda, lucro, margemRealPct };
      ...
      <div className="glass rounded-sm p-6 space-y-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="font-display text-2xl gold-text leading-none" data-testid="price-margin-value">
              {marginPct}%
            </span>
          </div>
      ...
      <div className="glass-strong rounded-sm p-6 relative overflow-hidden">
        ...
        <div className="relative space-y-4">
          ...
          <div className="border-t border-black/[0.08] pt-3 mt-2 space-y-2">
            ...
            <div className="p-4 rounded-sm bg-gradient-to-br from-gold-soft/40 to-gold/30 border border-gold/30 mt-1" data-testid="result-preco-venda">
...
function MeasureMode({ compact }) {
  ...
  const weightG = volPerMold * 1.1;

  return (
    <div className={gridCols(compact)} data-testid="calc-measure-panel">
      <div className="glass rounded-sm p-6 space-y-5">
        <div>
